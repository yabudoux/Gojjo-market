...
// Replace the existing POST /api/admin/hotels route with:
app.post('/api/admin/hotels', requireAdmin, upload.single('image'), (req, res) => {
    const { name, city, description, amenities, rating } = req.body;
...
// Replace the existing PUT /api/admin/hotels/:id route with:
app.put('/api/admin/hotels/:id', requireAdmin, upload.single('image'), (req, res) => {
    const hotelId = req.params.id;
    const { name, city, description, amenities, rating } = req.body;
    
    // First get current image to potentially delete it later
    db.get(`SELECT image_url FROM hotels WHERE id = ?`, [hotelId], (err, hotel) => {
        if (err) return res.status(500).json({ error: 'Failed to update hotel' });
        
        const image_url = req.file ? '/uploads/' + req.file.filename : req.body.image_url || hotel.image_url;
        ...
        db.run(`UPDATE hotels SET name = ?, city = ?, description = ?, amenities = ?, image_url = ?, rating = ? 
            ...
            [name, city, description, amenities, image_url, rating, hotelId], function(err) {
                ...
                if (req.file && hotel.image_url) {
                    const oldImagePath = path.join('public', hotel.image_url);
